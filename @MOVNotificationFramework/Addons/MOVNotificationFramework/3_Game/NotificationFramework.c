class NotificationMeta
{
	...
	{
		this.m_Queue = GetGame().GetCallQueue(CALL_CATEGORY_GAMEPLAY); 
		
		this.m_Alert = GetGame().GetWorkspace().CreateWidgets( "mov/scripts/layouts/alert.layout" );
		this.m_AlertIcon = ImageWidget.Cast( this.m_Alert.FindAnyWidget("NotificationImage") );
		this.m_AlertText = TextWidget.Cast( this.m_Alert.FindAnyWidget("NotificationText") );
		
		this.m_Alert.Show(false);
		this.m_AlertIcon.LoadImageFile(0, "mov/scripts/images/alert.paa");
		this.m_AlertText.SetText("");		
	}
}
